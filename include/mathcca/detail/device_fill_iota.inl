...
namespace mathcca {
  ...
  namespace algocca {
    ...
    void fill_iota(mathcca::iterator::device_iterator<T> first, mathcca::iterator::device_iterator<T> last, const T& v, cudaStream_t stream) {
      ...
      fill_iota_kernel<value_type><<<dimGrid, dimBlock, 0, stream>>>(first.get(), size, v);
    }
